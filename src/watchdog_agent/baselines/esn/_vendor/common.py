...
# commit 1b3e07fee53ae13407173c3ea932adb4a43e8230, file derail/common.py
# Copyright 2026 Sunny Dubey. Apache License 2.0: see LICENSE and NOTICE in this directory.
# Modified by watchdog (2026-09-25): this header added. No other change.
...
# --------------------------------------------------------------------------
# OPTIONAL goal-consistency channel c_t (telemetry v5, dims [60, 64)).
# Only episodes built with telemetry.adapter goal=True have it (which implies
# grounding=True). It asks a question no other channel can: not "is this step
# unusual" but "is this step still about the task", by measuring whether the
# content of each action is accounted for by the GOAL or by earlier tool
# OUTPUT. That is what makes a competently executed wrong task observable --
# such a run's latencies, action types and step-to-step drift are ordinary,
# which is why goal_drift episodes sit inside the healthy null of e/u/m/x.
# Every dim is causal, in [0, 1], and oriented so HIGHER = more anomalous.
# Appended above the grounding block (DESIGN.md amendment 8), so dims
# [0, 60) are byte-identical to telemetry v4 and every frozen artifact
# keeps its meaning.
# Traces with no recorded task text leave all four dims 0 (inert), never an
# error -- see telemetry.goal_consistency.
# --------------------------------------------------------------------------
D_GOAL = 4
D_TOTAL_GOAL = D_TOTAL_GRD + D_GOAL  # 64

GOAL_SLICE = slice(D_TOTAL_GRD, D_TOTAL_GOAL)  # c_t (derived, goal-relative)

IDX_GOAL_ARG_ABANDON = 60   # share of the tool ARGUMENTS the agent sent that
                            # the goal does not account for; held across steps
                            # that issue no call, so finishing is not drift
IDX_GOAL_ARG_ECHO = 61      # the part of that share which EARLIER tool output
                            # does account for — the agent is sourcing its
                            # actions from what it read, not from its task
IDX_GOAL_PROSE_ABANDON = 62  # same measure on the agent's own prose: catches
                            # answer-level contamination when the calls stay
                            # on-goal
IDX_GOAL_PROSE_ECHO = 63    # environment echo in that prose

CHANNEL_SLICES["c"] = GOAL_SLICE

# --------------------------------------------------------------------------
# OPTIONAL tool-reliability channel r_t (telemetry v6, dims [64, 71)).
# Only episodes built with telemetry.adapter reliability=True have it (which
# implies goal=True). `error_flag` is per-step and memoryless, so an isolated
# flake and the third step of a cascade look alike; healthy runs already carry
# errors on ~9% of steps, so error presence has almost no contrast. This
# channel carries the SHAPE of failure instead: its scope, its repetition and
# its failure to clear. Appended above the goal block, so dims [0, 64) are
# byte-identical to telemetry v5 and every frozen artifact keeps its meaning.
# Causal, bounded in [0, 1], HIGHER = more anomalous; a trace with no tool
# events leaves all seven dims 0 (inert), never an error.
# --------------------------------------------------------------------------
D_REL = 7
D_TOTAL_REL = D_TOTAL_GOAL + D_REL  # 71

REL_SLICE = slice(D_TOTAL_GOAL, D_TOTAL_REL)  # r_t (derived, tool reliability)

IDX_REL_FAIL_FRAC = 64        # share of THIS step's calls that errored
IDX_REL_DISTINCT_FAILING = 65  # distinct tools failing and not yet recovered,
                               # over distinct tools used so far
IDX_REL_REPEAT_FAIL = 66      # share of this step's failures whose exact
                              # (name, args) already failed earlier: a retry
                              # that fails again is not flakiness
IDX_REL_CONSEC_FAIL = 67      # consecutive failing calls across steps
IDX_REL_SINCE_CLEAN = 68      # steps since the last successful tool result;
                              # runs only after the episode's first tool call
IDX_REL_NO_RECOVERY = 69      # share of tools that failed and never recovered
IDX_REL_BURST = 70            # mean failure fraction over the last 3 steps

CHANNEL_SLICES["r"] = REL_SLICE

#: Declared semantics of a channel's DIMS, taken from the telemetry design
#: above and nothing else. A channel appears here only if its own definition
#: states both that every dim is bounded and that higher means more anomalous:
#: c ("Every dim is causal, in [0, 1], and oriented so HIGHER = more
#: anomalous") and r ("bounded in [0, 1], oriented so HIGHER = more
#: anomalous"). g declares the orientation but not a bound, so it is absent --
#: a monotone channel with no declared full scale gives no admissible unit.
#:
#: This is configuration, not evidence: it is read at fit time to decide how a
#: channel whose healthy null is constant may still be scored, and it never
#: consults a failure label, an onset, a corpus or an outcome.
MONOTONE_BOUNDED_CHANNELS: dict[str, tuple[float, float]] = {
    "c": (0.0, 1.0),
    "r": (0.0, 1.0),
}
